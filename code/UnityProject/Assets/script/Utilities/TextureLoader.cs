...
namespace AsImpL
{
    ...
    public class TextureLoader : MonoBehaviour
    {
        ...
        public static Texture2D LoadTexture(WWW www,string extension)
        {
            if (extension == ".dds")
            {
                Texture2D returnTex = LoadDDSManual(www.bytes);
                returnTex.name = www.url;
                return returnTex;
            }
            ...
            {
                Debug.Log("texture not supported : " + www.url);
            }
            return null;
        }
        ...
        /// <returns>The loaded texture or null on error.</returns>
        public static Texture2D LoadDDSManual(byte[] ddsBytes)
        {
            try
            {
