...
namespace AsImpL
{
    ...
    public abstract class Loader : MonoBehaviour
    {
        ...
        public IEnumerator Load(string objName, string objectId, Transform parentObj)
        {
            string name = objName;

            totalProgress.fileProgress.Add(objLoadingProgress);
            objLoadingProgress.fileName = objName;
            objLoadingProgress.error = false;
            objLoadingProgress.message = "Loading " + objName + "...";
            Debug.LogFormat("Loading {0}\n  from: {1}...", objName, objectId);

            yield return null;
            // if the model was already loaded duplicate the existing object
            if (buildOptions != null && buildOptions.reuseLoaded && loadedModels.ContainsKey(objectId) && loadedModels[objectId] != null)
            {
                Debug.LogFormat("File {0} already loaded, creating instance.", objectId);
                instanceCount[objectId]++;
                if (name == null || name == "") objName = objName + "_" + instanceCount[objectId];
                objLoadingProgress.message = "Instantiating " + objName + "...";
                while (loadedModels[objectId] == null)
                {
                    yield return null;
                }
                ...
                OnLoaded(newObj, objectId);
                yield break;
            }
            loadedModels[objectId] = null; // define a key for the dictionary
            instanceCount[objectId] = 0; // define a key for the dictionary

            float lastTime = Time.realtimeSinceStartup;
            float startTime = lastTime;
            yield return LoadModelFile(objectId);
            loadStats.modelParseTime = Time.realtimeSinceStartup - lastTime;

            if (objLoadingProgress.error)
            {
                OnLoadFailed(objectId);
                yield break;
            }
            ...
            {
                yield return LoadMaterialLibrary(objectId);
            }
            loadStats.materialsParseTime = Time.realtimeSinceStartup - lastTime;
            lastTime = Time.realtimeSinceStartup;
            yield return Build(objectId, objName, parentObj);
        ...
        /// <remarks>This is called by Load() method</remarks>
        protected IEnumerator Build(string objectId, string objName, Transform parentTransform)
        {
                ...
                foreach (MaterialData mtl in materialData)
                {
                    ...
                    if (mtl.diffuseTexPath != null)
                    {
...
#endif
                        {
                            ...
                            string texPath = GetTextureUrl(objectId, mtl.diffuseTexPath);
                            ...
                            {
                                mtl.diffuseTex = LoadTexture(loader, mtl.diffuseTexPath);
                            }
                        }
                    }
                    ...
                    if (mtl.bumpTexPath != null)
                    {
...
#endif
                        {
                            ...
                            string texPath = GetTextureUrl(objectId, mtl.bumpTexPath);
                            loader = new WWW(texPath);
                            yield return loader;
                            ...
                            {
                                mtl.bumpTex = LoadTexture(loader, mtl.bumpTexPath);
                            }
                        }
                    }
                    ...
                    if (mtl.specularTexPath != null)
                    {
...
#endif
                        {
                            ...
                            string texPath = GetTextureUrl(objectId, mtl.specularTexPath);
                            loader = new WWW(texPath);
                            yield return loader;
                            ...
                            {
                                mtl.specularTex = LoadTexture(loader, mtl.specularTexPath);
                            }
                        }
                    }
                    ...
                    if (mtl.opacityTexPath != null)
                    {
...
#endif
                        {
                            ...
                            string texPath = GetTextureUrl(objectId, mtl.opacityTexPath);
                            loader = new WWW(texPath);
                            yield return loader;
                            ...
                            {
                                mtl.opacityTex = LoadTexture(loader, mtl.opacityTexPath);
                            }
                        }
                    }
                }
        ...
        private Texture2D LoadTexture(WWW loader,string fileName)
        {
            ...
            if (ext == ".tga" || ext == ".dds")
            {
                tex = TextureLoader.LoadTexture(loader,ext);
